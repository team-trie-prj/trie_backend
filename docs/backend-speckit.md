...
- **F11. 시스템 프롬프트 인젝션 해킹 1차 필터링**
  - 규칙(정규식, KO/EN) 기반 1차 탐지 — 업로드 문서 본문 자동 차단(RAG 간접 주입 방어) + `POST /security/prompt-check`(질의 사전검사)
  - 심층 LLM 기반 2차 탐지는 vikira/범위 밖
...
- **F8. 매 쿼리 난수 세션 UUID + 검색·보고서 이력 로깅/복원 (FNC-HIS-01)**
  - 검색 요청마다 **난수 세션 UUID 발급** → 동일 결과 반복(캐시) 오류 차단
  - vikira `/search`·`/reports` **앞단 미들웨어**로 입력·VLM 분석·공공 통계·**보고서 스냅샷**을 동일 UUID 에 **자동 로깅**(비침투, PostgreSQL)
  - `GET /history`(목록) · `GET /history/{session_uuid}`(**캐시 무효화 후 1회성 복원**) · `DELETE`
  - 사용자당 **50개 상한, 최고참부터 FIFO 삭제**
