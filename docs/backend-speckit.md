...
- **F8. 매 쿼리 난수 세션 UUID + 검색 이력 로깅/복원 (FNC-HIS-01)**
  - 검색 요청마다 **난수 세션 UUID 발급** → 동일 결과 반복(캐시) 오류 차단
  - vikira `/search` **앞단 미들웨어**로 검색 트랜잭션 **자동 로깅**(비침투, session_uuid=`X-Session-Id`)
  - `GET /history`(목록) · `GET /history/{session_uuid}`(스냅샷 복원) · `DELETE` · **사용자별 FIFO** 상한
