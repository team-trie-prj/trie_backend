...
@router.post("", response_model=ApiResponse[SessionUUIDOut])
def create_session_uuid(response: Response) -> dict:
    response.headers["Cache-Control"] = "no-store, no-cache, must-revalidate"
    response.headers["Pragma"] = "no-cache"
    return ok(SessionUUIDOut(session_uuid=str(uuid.uuid4()), issued_at=utcnow()))
